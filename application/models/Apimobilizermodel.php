...
//#################### Add Student ####################//
  ...
  $transfer_certificate,$status,$created_by,$created_at,$father_mobile,$mother_mobile,$qualification,$qualification_details,$year_of_edu,$year_of_pass,$identification_mark_1,$identification_mark_2,$lang_known,$head_family_name,$head_family_edu,$no_family,$yearly_income,$jobcard_type)
	{
...
//#################### List Students ####################//
	...
	{
		 	$student_query = "SELECT id,name,sex,mobile,email,enrollment,status,student_pic,aadhaar_card_number FROM edu_student_prospects WHERE created_by  ='$user_id'";
			$student_res = $this->db->query($student_query);
        ...
        foreach($query_result as $rows_list){
          ...
          $student_result[]=array(
            ...
            "aadhaar_card_number"=>$rows_list->aadhaar_card_number,
...
//#################### List Students ####################//
	...
	{
		 	$student_query = "SELECT id,name,sex,mobile,email,enrollment,status,student_pic,aadhaar_card_number FROM edu_student_prospects WHERE created_by = '$user_id' AND status = '$status'";
			$student_res = $this->db->query($student_query);
			$query_result= $student_res->result();
			$student_count = $student_res->num_rows();
	...
	public function viewStudent($admission_id)
	{
        ...
        foreach($student_result as $rows_student){}
          $response_student_list=array(
            ...
            "transfer_certificate"=>$rows_student->edu_certificate,
...
//#################### Upload Documents upload ####################//
    ...
    function document_details_upload($user_id,$userFileName,$doc_master_id,$prospect_id,$proof_number){
      ...
      $result = $this->db->query($query);
			if($result) {
			    $response = array("status" => "success", "msg" => "Document uploaded","doc_file_name"=>$userFileName);
			}else{
				$response = array("status" => "error");
			}
